...
router.get('/summary', (req, res) => {
  ...
  // Server-side group aggregates (SBS). Display-only: returned in a separate map
  // keyed by group id, never injected into `programmes`. Group avg_daily_listens uses
  // the period-wide totalPeriodDays divisor (matches AccountView.jsx:534-562).
  const progMap = {};
  for (const r of rows) progMap[r.account_name] = r;
  const groupAggregates = {};
  for (const group of getAccountGroups('ga_listens')) {
    const memberNames = group.members.map(k => k.split('::')[0]);
    const memberRows = memberNames.map(n => progMap[n]).filter(Boolean);
    const total_listens = memberRows.reduce((s, p) => s + (p.total_listens || 0), 0);
    ...
    groupAggregates[group.id] = {
      ...
      total_listens,
      month_count,
      avg_daily_listens: avgPerDay(total_listens, totalPeriodDays, 1),
    };
  }
