...
const FB_METRICS = [
  ...
  { name: 'Unika tittare', key: 'account_viewers', desc: 'Hur många unika konton som sett något av kontots innehåll minst en gång under en hel månad. Metas nya mått från juni 2026 — ersätter Kontoräckvidd. En visning räknas när innehållet syns på minst halva skärmen i minst 0,25 sekunder, och samma konto räknas bara en gång per månad. Inkluderar både organiskt och betalt. Hämtas separat från Meta via Graph API.', source: 'API-export (Unika tittare)', summable: 'Nej — kan inte summeras meningsfullt' },
  { name: 'Kontoräckvidd (äldre mått)', key: 'account_reach', desc: 'DEPREKERAT — avvecklat av Meta i juni 2026; sista månaden med data är maj 2026. Mätte hur många unika personer som fått kontots innehåll levererat till sin skärm under en månad. Historiken behålls i appen men serien fortsätter inte. Jämför inte nivåerna med Unika tittare: gamla måttet räknade leverans till skärmen medan Unika tittare kräver en faktisk visning — en nivåskillnad vid bytet beror på måttdefinitionen, inte på att publiken förändrats.', source: 'API-export (historik)', summable: 'Nej — kan inte summeras meningsfullt' },
  {
...
const IG_METRICS = [
  ...
  { name: 'Unika tittare', key: 'ig_account_reach', desc: 'Hur många unika konton som sett kontots innehåll under en hel månad (hette tidigare Kontoräckvidd — samma mått, nytt namn för att matcha Facebooks Unika tittare; serien är obruten). Hämtas separat från Meta via Graph API, inte från exportfilerna för enskilda inlägg. Inkluderar både organiskt och betalt. OBS: API-uttag täcker max 30 dagar — 31-dagarsmånader kan vara cirka 1 % lägre än verkligt värde.', source: 'API-export', summable: 'Nej — kan inte summeras meningsfullt' },
